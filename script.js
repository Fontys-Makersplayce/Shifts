...
function checkFirebase() {
    if (typeof firebase !== 'undefined') {
        console.log("Firebase is now defined!", firebase);
        initializeActualApp();
    } else {
        console.log("Firebase is still not defined. Retrying in 100ms.");
...
function initializeActualApp() {
    console.log("initializeActualApp() is called, as Firebase is now defined.");
    ...
    let allShiftsData = {};
    let currentPastShiftViewDate = new Date();
    ...
    // Past Shifts Modal Elements
    const myShiftHistoryBtn = document.getElementById('myShiftHistoryBtn');
    const myPastShiftsModal = document.getElementById('myPastShiftsModal');
    const pastShiftsViewTypeSelectModal = document.getElementById('pastShiftsViewTypeSelectModal');
    const pastShiftsNavigationContainerModal = document.getElementById('pastShiftsNavigationContainerModal');
    const prevPastPeriodBtnModal = document.getElementById('prevPastPeriodBtnModal');
    const nextPastPeriodBtnModal = document.getElementById('nextPastPeriodBtnModal');
    const myPastShiftsPeriodDisplayModal = document.getElementById('myPastShiftsPeriodDisplayModal');
    const myPastShiftsTableBodyModal = document.getElementById('myPastShiftsTableModal').getElementsByTagName('tbody')[0];
    const closeMyPastShiftsModalBtn = document.getElementById('closeMyPastShiftsModalBtn');
    ...
    if (customPromptOkBtn) { customPromptOkBtn.addEventListener('click', () => { customPromptModal.classList.add('hidden'); customPromptModal.style.display = 'none'; if (currentConfirmResolve) { currentConfirmResolve(customPromptInput.value); currentPromptResolve = null; } }); }
    ...
    function getTodayMonday() {
        ...
        monday.setHours(0,0,0,0); // Ensure hours, minutes, seconds are set to 0.
        return monday;
    }
    ...
    // Helper function to calculate duration in hours from a timespan (e.g., "9:30 - 13:00")
    function calculateDuration(timeStr) {
        if (!timeStr || typeof timeStr !== 'string') return 0;
        const parts = timeStr.split(' - ');
        if (parts.length === 2) {
            const [startH, startM] = parts[0].split(':').map(Number);
            let [endH, endM] = parts[1].split(':').map(Number); // let, as endDate might be manipulated
            if (!isNaN(startH) && !isNaN(startM) && !isNaN(endH) && !isNaN(endM)) {
                const startDate = new Date(0, 0, 0, startH, startM);
                let endDate = new Date(0, 0, 0, endH, endM);
                if (endDate < startDate) { // If the shift goes past midnight
    ...
    function getShiftEndDateTime(shift) {
        if (!shift || !shift.date || !shift.time) return null;
        if (shift.description === "History" || shift.time.includes("Stunden") || shift.time.includes("hours")) return new Date(shift.date); // History/Stunden/Hours shifts are always in the past
        const timeParts = shift.time.split(' - ');
        if (timeParts.length < 2) return null;
        const endTimeStr = timeParts[1];
        const [endHour, endMinute] = endTimeStr.split(':').map(Number);
        if (isNaN(endHour) || isNaN(endMinute)) return null;
    ...
    async function sendEmailNotification(recipientEmail, subject, htmlBody, textBody) {
        console.log("sendEmailNotification called with recipient:", recipientEmail, "subject:", subject);
        if (!db) {
            console.error("ERROR in sendEmailNotification: Firestore DB (db) is not initialized. Cannot send email.");
            return;
        }
        try {
            console.log("Trying to add document to 'mail' collection for:", recipientEmail);
            ...
            console.log(`Email document for ${recipientEmail} successfully added to 'mail' collection.`);
        } catch (error) {
            console.error("ERROR adding email document to collection in sendEmailNotification:", error);
        }
    }
    ...
    async function initializeApp() {
        try {
            ...
            currentPastShiftViewDate = new Date();
    ...
    async function loadMasterPassword() {
        ...
        try {
            ...
            if (doc.exists && doc.data().masterAdminPassword) {
                ...
                console.warn("Admin config or master password not found. Using default.");
            }
        } catch (error) {
            console.error("Error loading master password (expected if rules are strict):", error.message);
        }
    }
        ...
        try {
            ...
            if (upcomingShifts.length > 0) {
                ...
                upcomingShifts.forEach(shift => {
                    ...
                    // MODIFIED: Directly display the time range instead of calculating hours.
                    ...
                    if (shiftsInPendingTrades.includes(shift.id)) {
                        ...
                        tradeBtn.classList.add('edit', 'small-action');
    ...
    async function renderTradeRequests() { // FIX: No more parameters needed
        ...
        try {
            // FIX: Execute queries directly here
    ...
    function renderUserShifts(shiftsForWeek, forWeekDate) {
        ...
        shiftsForWeek.forEach(shift => {
            const isStandard = standardShiftTimes.includes(shift.time);
            ...
            // Display hours for regular shifts
            if (shift.time && !shift.time.includes("hours") && !shift.time.includes("Stunden")) { // If it's not a history shift
                const duration = calculateDuration(shift.time);
                if (duration > 0) {
                    timeCell.textContent = `${duration.toFixed(2)} hours`;
                } else {
                    timeCell.textContent = shift.time; // Fallback if duration is not calculable
                }
            } else { // For history shifts or already hour-formatted times
                timeCell.textContent = shift.time;
            }
    ...
    function renderUserPublishedSchedule(shiftsForWeek, tableBodyElement) {
        ...
        const standardApprovedShifts = shiftsForWeek.filter(s => s.status === "approved" && standardShiftTimes.includes(s.time));
        ...
        workDays.forEach(day => {
            ...
            standardShiftTimes.forEach(time => {
                ...
                // Display hours for standard shifts in Published Schedule
                const duration = calculateDuration(time);
                if (duration > 0) {
                    cell.textContent = shiftInfo.names.length > 0 ? shiftInfo.names.join(', ') : "---";
                    const hoursSpan = document.createElement('span');
                    hoursSpan.classList.add('shift-description-display'); // Reuse this class for styling
                    hoursSpan.textContent = `(${duration.toFixed(2)} hours)`;
    ...
    function renderLatestCustomShiftsForUser(shiftsForWeek) {
        ...
        const customApprovedShifts = shiftsForWeek.filter(s =>
            s.status === "approved" &&
            !standardShiftTimes.includes(s.time) &&
        ...
        if (customApprovedShifts.length > 0) {
            ...
            customApprovedShifts.forEach(shift => {
                ...
                row.insertCell().textContent = shift.day;
                row.insertCell().textContent = formatDate(new Date(shift.date), false);
                row.insertCell().textContent = shift.time; // MODIFIED: Directly display the time range
                row.insertCell().textContent = shift.description || "N/A";
            ...
            userLatestCustomShiftsSection.classList.add('hidden');
        }
    }
function renderScheduleOverview(shiftsForWeek, tableBodyElement) {
    ...
    workDays.forEach(day => {
        ...
        const currentDayAsDate = new Date(dateForDay);
        currentDayAsDate.setHours(0,0,0,0);
        const isDayPast = currentDayAsDate.getTime() < getTodayMonday().getTime();
        ...
        standardShiftTimes.forEach(time => {
            ...
            const shiftInfo = scheduleData[day][time];
            const shiftObject = shiftsForWeek.find(s => s.day === day && s.time === time);

            if (shiftInfo.exists && shiftObject) {
                const shiftEnd = getShiftEndDateTime(shiftObject);
                const isIndividualShiftPast = shiftEnd && shiftEnd <= now;
                ...
                deleteBtn.title = (isIndividualShiftPast || isDayPast) ? 'This shift is in the past and cannot be deleted.' : 'Delete this shift';
                deleteBtn.disabled = isIndividualShiftPast || isDayPast;
                // FIX: Added the missing onclick event handler for deleting shifts
                deleteBtn.onclick = () => managerAddOrDeleteStandardShift(day, time, 'delete');
                ...
                addBtn.title = isDayPast ? 'Past days cannot be changed.' : 'Add this shift';
                addBtn.disabled = isDayPast;
                // FIX: Added the missing onclick event handler for adding shifts
                addBtn.onclick = () => managerAddOrDeleteStandardShift(day, time, 'add');
                cell.appendChild(addBtn);
            }
        });
    });

    const customShifts = shiftsForWeek.filter(s => !standardShiftTimes.includes(s.time));
    if (managerCustomShiftsPreviewTableBody) {
        managerCustomShiftsPreviewTableBody.innerHTML = '';
        const filteredCustomShifts = customShifts.filter(s => s.description !== "History");

        if (filteredCustomShifts.length > 0) {
            managerCustomShiftsPreviewSection.classList.remove('hidden');
            sortShiftsArray(filteredCustomShifts);
            filteredCustomShifts.forEach(shift => {
                const shiftEnd = getShiftEndDateTime(shift);
                const isPast = shiftEnd && shiftEnd <= now;
                ...
                row.insertCell().textContent = (Array.isArray(shift.assignedCoworkers) && shift.assignedCoworkers.length > 0) ? shift.assignedCoworkers.join(', ') : "Unassigned";
                const actionCell = row.insertCell();
                ...
                deleteBtn.textContent = 'x';
                deleteBtn.classList.add('delete', 'small-action');
                deleteBtn.title = isPast ? 'This event is in the past and cannot be deleted.' : 'Delete this custom shift/event';
                deleteBtn.disabled = isPast;
                deleteBtn.onclick = (e) => {
                    e.stopPropagation();
                    handleDeleteCustomShift(shift.id, getWeekKey(new Date(shift.date)));
                };
    ...
    async function refreshManagerDashboard() {
        ...
        try {
            ...
            const isViewingWeekPast = currentViewingDateMgr.getTime() < todayMonday.getTime();
            ...
            publishWeekBtn.disabled = isViewingWeekPast;
            clearWeekShiftsBtn.disabled = isViewingWeekPast;
            showCustomShiftModalBtn.disabled = isViewingWeekPast;
...
function renderManagerApplications(allShiftsForWeek) {
    // *** FIX: Simplified and corrected the filtering logic ***
    const shiftsToDisplay = allShiftsForWeek.filter(shift => {
        // A shift is standard if its time string exactly matches one of the standard times.
        const isStandard = standardShiftTimes.includes(shift.time);

        // A shift is a special history entry if its description is "History". These should not appear in application management.
        const isHistoryEntry = shift.description === "History";
        if (isHistoryEntry) {
            return false;
        }
    ...
    if (shiftsToDisplay.length === 0) {
        targetTableBody.innerHTML = `<tr><td colspan="6">No ${currentManagerShiftView} shifts for this week.</td></tr>`;
    } else {
        ...
        shiftsToDisplay.forEach(shift => {
            ...
            if (shift.time && !shift.time.includes("hours") && !shift.time.includes("Stunden")) {
                const duration = calculateDuration(shift.time);
                if (duration > 0) {
                    timeCell.textContent = `${duration.toFixed(2)} hours`;
                } else {
            ...
            if (shift.status === "pending" && Array.isArray(shift.applicants) && shift.applicants.length > 0) {
                ...
                approveSelectedBtn.disabled = isPast;
                ...
                editBtn.disabled = isPast;
                editBtn.title = isPast ? 'Cannot edit a past shift.' : 'Edit assignments';
                ...
                clearBtn.disabled = isPast;
                clearBtn.title = isPast ? 'Cannot clear a past shift.' : 'Clear assignments';
                ...
                assignBtn.disabled = isPast;
                assignBtn.title = isPast ? 'Cannot assign to a past shift.' : 'Assign coworkers';
    ...
    function renderHoursOverview(shiftsForViewingWeek) {
        ...
        const processShiftHours = (shift) => {
            if (shift.time && (shift.time.includes("Stunden") || shift.time.includes("hours"))) {
                const hoursStr = String(shift.time).split(' ')[0].replace(',', '.');
                return parseFloat(hoursStr) || 0;
            }
            if (shift.time && shift.time.includes(' - ')) {
                return calculateDuration(shift.time);
            }
            return 0; // Fallback
        };
        ...
        // *** FIX: Simplified and corrected the filtering logic to match renderManagerApplications ***
        const filteredShiftsForHours = shiftsToProcessForHours.filter(s => {
            const isStandard = standardShiftTimes.includes(s.time);
            const isHistoryEntry = s.description === "History";
            // For hour calculations, we ignore "History" entries.
            if (isHistoryEntry) {
                return false;
            }
    ...
    async function renderMyPastShiftsModal() {
        if (!loggedInCoworker) return;

        myPastShiftsTableBodyModal.innerHTML = `<tr><td colspan="3">Loading your shift history...</td></tr>`;
        const viewType = pastShiftsViewTypeSelectModal.value;

        pastShiftsNavigationContainerModal.classList.toggle('hidden', viewType === 'total');

        let periodStart, periodEnd;
        let displayPeriodString = "";

        switch (viewType) {
            case "weekly":
                periodStart = getMondayOfDate(new Date(currentPastShiftViewDate));
                periodEnd = new Date(periodStart);
                periodEnd.setDate(periodStart.getDate() + 6);
                displayPeriodString = `Week of ${formatDate(periodStart, false)}`;
                break;
            case "monthly":
                periodStart = new Date(currentPastShiftViewDate.getFullYear(), currentPastShiftViewDate.getMonth(), 1);
                periodEnd = new Date(currentPastShiftViewDate.getFullYear(), currentPastShiftViewDate.getMonth() + 1, 0);
                displayPeriodString = `${periodStart.toLocaleString('default', { month: 'long' })} ${periodStart.getFullYear()}`;
                break;
            case "yearly":
                periodStart = new Date(currentPastShiftViewDate.getFullYear(), 0, 1);
                periodEnd = new Date(currentPastShiftViewDate.getFullYear(), 11, 31);
                displayPeriodString = `Year ${currentPastShiftViewDate.getFullYear()}`;
                break;
            case "total":
                displayPeriodString = "All Time";
                break;
        }
        myPastShiftsPeriodDisplayModal.textContent = displayPeriodString;
        ...
        try {
            ...
            querySnapshot.forEach(doc => {
                ...
                if (weekData && Array.isArray(weekData.shifts)) {
                    const userShiftsThisWeek = weekData.shifts.filter(shift =>
                        shift.status === 'approved' &&
                        Array.isArray(shift.assignedCoworkers) &&
                        shift.assignedCoworkers.includes(loggedInCoworker.name)
                    );
                    allUserShifts.push(...userShiftsThisWeek);
                }
            });

            const now = new Date();

            const trulyPastShifts = allUserShifts.filter(shift => {
                if (shift.description === "History" || shift.time.includes("Stunden") || shift.time.includes("hours")) { // Recognition of the "History" format
                    return true;
                }
                const shiftEndDateTime = getShiftEndDateTime(shift);
                return shiftEndDateTime && shiftEndDateTime <= now;
            });

            let filteredShifts = trulyPastShifts;
            if (viewType !== 'total') {
                periodEnd.setHours(23, 59, 59, 999);
                filteredShifts = trulyPastShifts.filter(shift => {
                    const shiftDate = new Date(shift.date);
                    return shiftDate >= periodStart && shiftDate <= periodEnd;
                });
            }

            myPastShiftsTableBodyModal.innerHTML = "";
            if (filteredShifts.length === 0) {
                myPastShiftsTableBodyModal.innerHTML = `<tr><td colspan="3">No approved shifts found for this period.</td></tr>`;
                return;
            }

            sortShiftsArray(filteredShifts);
            filteredShifts.forEach(shift => {
                const row = myPastShiftsTableBodyModal.insertRow();
                row.insertCell().textContent = formatDate(new Date(shift.date), false);
                row.insertCell().textContent = shift.day;
                const timeCell = row.insertCell();

                if (shift.time) {
                    let displayTime = shift.time;
                    if (shift.time.includes(" - ")) { // Standard or custom time range
                        const duration = calculateDuration(shift.time);
                        if (duration > 0) {
                            displayTime = `${duration.toFixed(2)} hours`;
                        }
                    } // Otherwise it's already X.XX hours from import
                    timeCell.textContent = displayTime;
                }

                if (shift.description && shift.description !== "Normal Shift" && shift.description !== "History") {
                    const descSpan = document.createElement('span');
                    descSpan.className = 'shift-description-display';
                    descSpan.textContent = shift.description;
                    timeCell.appendChild(descSpan);
                } else if (shift.description === "History") { // If it's explicitly "History", display that
                    const descSpan = document.createElement('span');
                    descSpan.className = 'shift-description-display';
                    descSpan.textContent = "History";
                    timeCell.appendChild(descSpan);
                }
            });
            ...
            myPastShiftsTableBodyModal.innerHTML = `<tr><td colspan="3" class="error-message">Error loading past shifts.</td></tr>`;
        }
    }
    ...
    async function managerAddOrDeleteStandardShift(day, time, action) {
        ...
        try {
            await db.runTransaction(async (transaction) => {
                ...
                const currentDayAsDate = new Date(shiftSpecificDateISO);
                currentDayAsDate.setHours(0,0,0,0);
                const isDayPast = currentDayAsDate.getTime() < getTodayMonday().getTime();

                if (action === 'add' && existingShiftIndex === -1) {
                    if (isDayPast) { await showAlert("Past days cannot be changed."); return; }
                    const newShift = {
                        id: `${weekKey}-${Date.now()}`,
                        day: day,
                    ...
                    const shiftEnd = getShiftEndDateTime(shiftToDelete);
                    const isIndividualShiftPast = shiftEnd && shiftEnd <= new Date();

                    if (isIndividualShiftPast || isDayPast) { await showAlert("Past shifts or days cannot be deleted."); return; }
            ...
            if (error.message !== "Deletion cancelled by user.") {
                 ...
                 await showAlert("Error modifying shift: " + error.message);
            }
        }
    }
    ...
    async function handleClearWeekShifts() {
        const todayMonday = getTodayMonday();
        const isViewingWeekPast = currentViewingDateMgr.getTime() < todayMonday.getTime();
        if (isViewingWeekPast) { await showAlert("Past weeks cannot be deleted."); return; }
        ...
        try {
            ...
            await showAlert("Error", "Could not delete shifts for this week.");
        }
    }

    async function handleDeleteCustomShift(shiftId, weekKey) {
        const confirmDelete = await showConfirm("Are you sure you want to delete this custom shift/event? This cannot be undone.");
        ...
        try {
            await db.runTransaction(async (transaction) => {
                const weekDoc = await transaction.get(weekDocRef);
                if (!weekDoc.exists) {
                    throw new Error("Week data not found, cannot delete shift.");
                }
                ...
                const initialShiftCount = weekData.shifts.length;
                weekData.shifts = weekData.shifts.filter(s => s.id !== shiftId);

                if (weekData.shifts.length === initialShiftCount) {
                    console.warn(`Shift with ID ${shiftId} was not found for deletion in week ${weekKey}. Maybe it was already deleted.`);
                }
            ...
            await showAlert("Success", "The custom shift/event has been deleted.");
            await refreshManagerDashboard();

        } catch (error) {
            console.error("Error deleting custom shift:", error);
            await showAlert("Error", "The custom shift/event could not be deleted. " + error.message);
        }
    }

    function isCustomTimeShift(timeStr) {
        if (!timeStr) return false;
        // This function is kept for legacy checks but is no longer the primary way to determine custom vs standard
        return !standardShiftTimes.includes(timeStr) && !timeStr.includes("Stunden") && !timeStr.includes("hours");
    }
    ...
    async function handleAddCoworker() {
        ...
        if (!name || !password) { await showAlert("Name and password are required."); return; }
        ...
        try {
            ...
            await showAlert("Error adding coworker: " + error.message);
        }
    }
    ...
    async function handleDeleteCoworker(coworkerDocIdOrName) {
        ...
        if (coworkerToDelete.isManager && coworkers.filter(c => c.isManager).length <= 1) {
            await showAlert("Action denied: The last manager cannot be deleted. The system requires at least one manager account.");
            return;
        }
        if (coworkerToDelete.isManager) {
            ...
            if (enteredPassword !== currentMasterPassword) { await showAlert("Incorrect master admin password. Deletion cancelled."); return; }
            await showAlert("Master admin password confirmed. Proceeding with deletion confirmation...");
        }

        const doDelete = await showConfirm(`Delete coworker ${coworkerToDelete.name}? This will also remove their assignments and applications.`);
        if (!doDelete) return;
        ...
        try {
            ...
            await showAlert("Error deleting coworker: " + error.message);
        }
    }
    ...
    async function handleToggleManagerStatus(coworkerDocIdOrName) {
        ...
        if (coworkerToToggle.isManager && newManagerStatus === false && coworkers.filter(c => c.isManager).length <= 1) {
            await showAlert("Action denied: The last manager cannot be demoted. The system requires at least one manager account.");
            return;
        }
        if (coworkerToToggle.isManager && newManagerStatus === false) {
            ...
            if (enteredPassword !== currentMasterPassword) { await showAlert("Incorrect master admin password. Demotion cancelled."); return; }
            await showAlert("Master admin password confirmed. Proceeding with demotion...");
        }

        try {
            await db.collection('coworkers').doc(coworkerToToggle.id || coworkerToToggle.name).update({ isManager: newManagerStatus });
            coworkerToToggle.isManager = newManagerStatus;
            renderCoworkerListForManager();
            if (loggedInCoworker && (loggedInCoworker.id === coworkerToToggle.id || loggedInCoworker.name === coworkerToToggle.name)) {
    ...
    async function handleResetAllData() {
        ...
        if (doReset) {
            ...
            allShiftsData = {};
            await initializeApp();
            await showAlert("Local session reset. App data re-initialized (may be fetched from Firestore or used defaults if Firestore is empty).");
        }
    }
    ...
    function openAssignEditModal(shift, weekKey, isEditingAssignments) {
        ...
        assignEditModalTitle.textContent = isEditingAssignments ? `Edit assignments for ${shift.day} ${shift.time}` : `Assign coworker to ${shift.day} ${shift.time}`;
    ...
    if(assignEditSaveBtn) {
        assignEditSaveBtn.addEventListener('click', async () => {
            ...
            } catch (error) { console.error("Error saving assignments from modal:", error); await showAlert("Error saving assignments: " + error.message); }
        });
    }
        ...
        saveCustomShiftBtn.addEventListener('click', async () => {
            ...
            try {
                await db.runTransaction(async (transaction) => {
                    ...
                    if (existingShift && !editingCustomShiftId) { throw new Error(`A custom shift for ${formatDate(selectedDate, false)} at ${shiftTimeStr} with the description "${description}" already exists.`); }
                ...
                await prepareUserView();
            } catch (error) { console.error("Error adding custom shift:", error); await showAlert("Error saving custom shift: " + error.message); }
        });
    }
    ...
    if (emailPromptSaveBtn) {
        emailPromptSaveBtn.addEventListener('click', async () => {
            console.log("DEBUG: emailPromptSaveBtn clicked");
            ...
            if (!email) {
                emailPromptError.textContent = "Please enter an email address or skip this step.";
                return;
            }
            ...
            try {
                ...
                console.log("DEBUG: Email saved in coworker document (Prompt). Now trying to call sendEmailNotification for:", email);
                ...
                emailPromptError.textContent = "Error saving email. Please try again.";
            }
        });
    }
        ...
        emailPromptSkipBtn.addEventListener('click', async () => {
            ...
            if (dontRemind && loggedInCoworker) {
                try {
                    ...
                    console.error("Error setting 'do not remind' flag:", error);
                }
            }
            emailPromptModal.classList.add('hidden');
            emailPromptModal.style.display = 'none';
    ...
    if (myAccountBtn) {
        myAccountBtn.addEventListener('click', () => {
            ...
            [latestPublishedScheduleSection, userShiftView, userCustomShiftsSection, myUpcomingShiftsSection, shiftTradeRequestsSection].forEach(s => { if(s) s.classList.add('hidden'); });
    ...
    if (submitMyPasswordChangeBtn) {
        submitMyPasswordChangeBtn.addEventListener('click', async () => {
            ...
            if (!newPass || newPass.length < 4) { myPasswordChangeError.textContent = "New password must be at least 4 characters long."; return; }
            ...
            } catch (error) { console.error("Error changing own password:", error); myPasswordChangeError.textContent = "Error changing password. Please try again."; }
        });
    }

    if (saveMyEmailBtn) {
        saveMyEmailBtn.addEventListener('click', async () => {
            console.log("DEBUG: saveMyEmailBtn clicked");
            if (!loggedInCoworker) return;
            ...
            try {
                ...
                const localCoworker = coworkers.find(c => c.id === loggedInCoworker.id || c.name === loggedInCoworker.name);
                if (localCoworker) {
                    localCoworker.email = newEmail;
                }

                myEmailSuccess.textContent = "Email address saved successfully!";
                setTimeout(() => { myEmailSuccess.textContent = ""}, 3000);

                console.log("DEBUG: Email in coworker document updated. Now trying to call sendEmailNotification for:", newEmail);
                ...
                myEmailError.textContent = "Error saving email. Please try again.";
            }
        });
    }
    ...
    if (deleteMyEmailBtn) {
        deleteMyEmailBtn.addEventListener('click', async () => {
            ...
            } catch (error) { console.error("Error deleting email:", error); myEmailError.textContent = "Error deleting email. Please try again."; }
        });
    }
    ...
    if (sendTestEmailBtn) {
        sendTestEmailBtn.addEventListener('click', async () => {
            if (!loggedInCoworker || !loggedInCoworker.email) {
                ...
                await showAlert("Email Not Set", "Please set and save your email address in 'My Account Settings' before sending a test email.");
                return;
            }

            if(testEmailStatus) testEmailStatus.textContent = "Sending test email...";
            console.log("DEBUG: sendTestEmailBtn clicked. Sending test email to:", loggedInCoworker.email);

            const subject = "Test Email from Shift Scheduler Pro";
            const htmlBody = `<p>Hello ${loggedInCoworker.name},</p><p>This is a test email triggered from the 'Send Test Email to My Address' button in your account settings.</p><p>If you have received this, email sending is working!</p><p>Your registered email is: ${loggedInCoworker.email}</p>`;
            const textBody = `Hello ${loggedInCoworker.name},\n\nThis is a test email triggered from the 'Send Test Email to My Address' button in your account settings.\n\nIf you have received this, email sending is working!\n\nYour registered email is: ${loggedInCoworker.email}`;

            try {
                await sendEmailNotification(loggedInCoworker.email, subject, htmlBody, textBody);
                if(testEmailStatus) testEmailStatus.textContent = "Test email request sent! Check your inbox (and spam folder).";
                ...
                await showAlert("Error", "The test email could not be sent. Please check the console for errors.");
            }
        });
    }
    ...
    // --- New Event Listeners for Past Shifts Modal ---
    if (myShiftHistoryBtn) {
        myShiftHistoryBtn.addEventListener('click', () => {
            currentPastShiftViewDate = new Date();
            pastShiftsViewTypeSelectModal.value = 'monthly';
            myPastShiftsModal.classList.remove('hidden');
            myPastShiftsModal.style.display = 'flex';
            renderMyPastShiftsModal();
        });
    }
    if (closeMyPastShiftsModalBtn) {
        closeMyPastShiftsModalBtn.addEventListener('click', () => {
            myPastShiftsModal.classList.add('hidden');
            myPastShiftsModal.style.display = 'none';
        });
    }
    if (pastShiftsViewTypeSelectModal) {
        pastShiftsViewTypeSelectModal.addEventListener('change', () => {
            currentPastShiftViewDate = new Date();
            renderMyPastShiftsModal();
        });
    }
    if (prevPastPeriodBtnModal) {
        prevPastPeriodBtnModal.addEventListener('click', () => {
            const viewType = pastShiftsViewTypeSelectModal.value;
            if (viewType === "weekly") {
                currentPastShiftViewDate.setDate(currentPastShiftViewDate.getDate() - 7);
            } else if (viewType === "monthly") {
                currentPastShiftViewDate.setMonth(currentPastShiftViewDate.getMonth() - 1);
            } else if (viewType === "yearly") {
                currentPastShiftViewDate.setFullYear(currentPastShiftViewDate.getFullYear() - 1);
            }
            renderMyPastShiftsModal();
        });
    }
    if (nextPastPeriodBtnModal) {
        nextPastPeriodBtnModal.addEventListener('click', () => {
            const viewType = pastShiftsViewTypeSelectModal.value;
            if (viewType === "weekly") {
                currentPastShiftViewDate.setDate(currentPastShiftViewDate.getDate() + 7);
            } else if (viewType === "monthly") {
